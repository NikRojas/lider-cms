  ...
  methods: {
    handleEnd: function handleEnd(added, removed, moved) {
      ...
      setTimeout(function () {
        if (_this.files[added.moved.newIndex] instanceof File) {
          ...
          _this.$refs['ref_file_' + added.moved.newIndex][0].dropzone.emit('thumbnail', _this.files[added.moved.newIndex], _this.files[added.moved.newIndex].dataURL);

          _this.$refs['ref_file_' + added.moved.newIndex][0].dropzone.emit('complete', _this.files[added.moved.newIndex]);
        }
      }, 100);
    },
    add: function add() {
    ...
    handleAddedFiles: function handleAddedFiles(e, index) {
      ...
      setTimeout(function () {
        ...
        console.log();
        _this2.files[index] = _this2.$refs["ref_file_" + index][0].dropzone.files[0];
      }, 500);
    }
  },
  watch: {
                      ...
                      [
                        _c("ImageForm", {
                          attrs: {
                            ...
                            folder: "projects",
                            imagesUrl: _vm.imagesUrl,
                            valueEsParent: _vm.element.banner_en,
                            valueEnParent: _vm.element.banner_es,
