...
class ProjectsController extends Controller
{
    ...
    public function getAll(Request $request)
    {
        ...
        $elements = Project::with('statusRel', 'ubigeoRel','tipologiesRel','departmentsRel')->withCount('galleryRel','departmentsRel', 'filesRel', 'tipologiesRel', 'quotationsRel')->where('id', "!=", $not)->orderBy('index', 'asc')->get();
    ...
    public function store(ProjectRequest $request)
    {
        ...
        ,'reservation_in_package','package_description']);
        ...
        $cardHoverName = $this->setFileName('ch-', $request->file('card_hover'));
        $storeCardHover = Storage::disk('public')->putFileAs('img/projects/', $request->file('card_hover'), $cardHoverName);
        
        $cardName = $this->setFileName('c-', $request->file('card'));
        $storeCard = Storage::disk('public')->putFileAs('img/projects/', $request->file('card'), $cardName);
        //fin actualización
        ...
        array_push($array_cards, $cardHoverName);
        
        if (!$storeLogo || !$storeBanner || !$storeBrochure || !$storeLogoClour || !$storeMap || !$storeCardHover || !$storeCard) {
            ...
            //
            Storage::disk('public')->delete('img/projects/'.$cardName);
            Storage::disk('public')->delete('img/projects/'.$cardHoverName);
            //
        ...
        $project = array_merge($project, ["images" => json_encode($array_cards), "logo"=>$logoName, "iframe_map"=>$mapName, "logo_colour"=>$logoNameColour,"brochure"=>$brochureName,"index" => $index,"banner"=>$imageBanner]);
        try {
    ...
    public function update(ProjectRequest $request, Project $element)
    {
        ...
        if ($request->hasFile('card_hover')) {
            ...
            $card_hover =  $cardHoverName;
        } else {
            $card_hover =  $images_decode[1];
        }

        $array_cards = array();
        array_push($array_cards, $card);
        array_push($array_cards, $card_hover);
        //fin actualizacion
        ...
        //
        if ($request->hasFile('card') && $images_decode[0]) {
            Storage::disk('public')->delete('img/projects/'.$images_decode[0]);
        }
        if ($request->hasFile('card_hover') && $images_decode[1]) {
            Storage::disk('public')->delete('img/projects/'.$images_decode[1]);
        }
        //
