...
class PostController extends BaseController
{
    ...
    public function customerPlatformCommercial(CustomerRequest $request){
        ...
        if(!$request->oi){
            ...
            $r_order = ["advisor_id" => $advisorId,"sended_code_sap" => $decrypted_string, "customer_id" => $customer->id, "total_price" => $price_deparment_separation, "order_date" => Carbon::now(),'master_currency_id' => $project->master_currency_id]; 
            #Setear Asesor si viene desde la URL de Separacion
            try {
                $order = Order::UpdateOrCreate($r_order);
            }
