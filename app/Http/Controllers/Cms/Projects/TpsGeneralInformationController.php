...
class TpsGeneralInformationController extends Controller
{
  ...
  {
    $request_information = request(["tps_principal_title", "tps_latitude", "tps_longitude", "tps_location_description",'tps_base_dsct_ci','price_bill','tps_quotation_text','tps_url_model']);
    try {
  ...
  {
    $informacion = Project::select("id", "tps_principal_title",'price_bill','tps_url_model', "tps_latitude", "tps_longitude", "tps_location_description",'tps_base_dsct_ci','tps_quotation_text')->where('id', $request->project_id)->orderBy('created_at', 'desc')->first();
