  ...
  <div class="row mb-5">
    <div class="col-12 col-lg-12">
      <div class="text-right mb-3">
        ...
        >
          ...
          <span class="btn-inner--text">Nuevo Material</span>
        </a>
      </div>
      <DataTableDraggable
        ...
        @edit="editEl"
        @delete="deleteEl"
        :message-order="messageOrderMaterials"
      ></DataTableDraggable>
    </div>
    <b-modal
    ...
    >
      <template slot="modal-title">
        <div class="text-primary h2">{{ title }} Material</div>
      </template>
...
export default {
  ...
  methods: {
    ...
    orderElements(elements) {
      axios
        .put(this.routeOrderMaterials, elements)
        .then((response) => {
          this.restore();
          Swal.fire({
            title: response.data.title,
    ...
    destroyConfirm() {
      this.requestSubmit = true;
      axios
        .delete(this.routeMaterials + "/" + this.element.id)
        .then((response) => {
          this.restore();
          Swal.fire({
            title: response.data.title,
    ...
    getEl(id) {
      ...
      axios
        .get(this.routeMaterials + "/json/get/" + id)
        .then((response) => {
